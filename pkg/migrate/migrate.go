...
func Run(opts Options) (*Result, error) {
	...
	mobaiClawHome, err := resolveMobaiClawHome(opts.MobaiClawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, mobaiClawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to MobaiClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, mobaiClawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, mobaiClawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(mobaiClawHome, "config.json"),
				Description: "convert OpenClaw config to MobaiClaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(mobaiClawHome)
...
func Execute(actions []Action, openclawHome, mobaiClawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, mobaiClawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, mobaiClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing MobaiClaw config: %w", err)
		}
...
func resolveMobaiClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("MOBAICLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".mobaiclaw"), nil
}
