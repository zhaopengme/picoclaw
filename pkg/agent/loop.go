...
func NewAgentLoop(cfg *config.Config, msgBus bus.Broker, provider providers.LLMProvider) *AgentLoop {
	...
	// Set the tool registration function for agent instances
	SetupAgentTools = setupAgentTools

	return al
}

// setupAgentTools registers shared tools for a single agent instance.
// This is used both during initial registration and after config reload.
func setupAgentTools(agentID string, agent *AgentInstance, cfg *config.Config, msgBus bus.Broker, registry *AgentRegistry, provider providers.LLMProvider) {
	// Web tools
...
func registerSharedTools(cfg *config.Config, msgBus bus.Broker, registry *AgentRegistry, provider providers.LLMProvider) {
	for _, agentID := range registry.ListAgentIDs() {
		...
		setupAgentTools(agentID, agent, cfg, msgBus, registry, provider)
	}
}
...
func (al *AgentLoop) GetConfig() *config.Config {
	return al.cfg.Load().(*config.Config)
}

// UpdateConfig updates the config reference atomically.
func (al *AgentLoop) UpdateConfig(cfg *config.Config) {
	al.cfg.Store(cfg)
}

// UpdateProvider is a no-op for hot-reload compatibility.
// The provider is implicitly updated when AgentRegistry.Reload() recreates
// all AgentInstance instances with the new provider.
func (al *AgentLoop) UpdateProvider(provider providers.LLMProvider) {
	// No-op: provider is stored in AgentInstance, updated via registry reload
}
